...
func New(address string) *FrameworkAPIClient {
	client := &http.Client{
	...
	return &FrameworkAPIClient{endpoint, userAgent, client, bytes.NewBuffer(nil)}
}
...
func (c *FrameworkAPIClient) Next() (*Invoke, error) {
	url := c.baseURL + "next"
...
func (c *FrameworkAPIClient) post(url string, body io.Reader, contentType string) error {
